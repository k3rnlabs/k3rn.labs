...
-- qualify StudioCreation columns returned by the PL/pgSQL
-- function. "shotIndex" is also an output parameter of
-- RETURNS TABLE and must not remain an unqualified
-- identifier inside RETURN QUERY.
...
BEGIN
  ...
  IF
    ...
    OR jsonb_array_length(p_shots) <> 6
  THEN
    RAISE EXCEPTION
  ...
  FOR v_shot IN
    SELECT value
    FROM jsonb_array_elements(p_shots)
  LOOP
    v_index :=
      (v_shot->>'shotIndex')::INTEGER;
